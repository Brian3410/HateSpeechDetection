...
class Config:
    ...
    MODEL_OUTPUT_DIR = "distilbert_pos_only_model"
...
def train_model():
    """Enhanced training function for DistilBERT with POS features only (no SMOTE, no class weights)."""
    print("🚀 Starting DistilBERT Training with POS Features Only (No SMOTE, No Class Weights)")
    print("=" * 80)
    ...
    with open(report_path, 'w') as f:
        f.write("DistilBERT with POS Features Only - Hate Speech Detection Test Results\n")
        f.write("=" * 70 + "\n\n")
    ...
    print("\n" + "="*60)
    print("🎉 DISTILBERT WITH POS FEATURES ONLY TRAINING COMPLETED!")
    print("="*60)
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        Config.DATASET_PATH = sys.argv[1]
        Config.MODEL_OUTPUT_DIR = "distilbert_pos_only_" + os.path.splitext(os.path.basename(Config.DATASET_PATH))[0] + "_model"
    else:
        print("Usage: python train_distilbert_pos_only.py <dataset_path> <output_dir>")
        sys.exit(1)
