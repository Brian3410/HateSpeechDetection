...
def find_latest_checkpoint(output_dir):
    ...
    for cp in checkpoints:
        try:
            cp_num = int(os.path.basename(cp).split('-')[1])
            checkpoint_numbers.append((cp_num, cp))
...
def load_model_from_checkpoint(checkpoint_path, base_model_name="google/gemma-7b"):
    """Load model from checkpoint with proper PEFT handling."""
    ...
    try:
        ...
        # Wrap with POS features
        model = GemmaWithPOS(model, pos_feature_dim=Config.POS_FEATURE_DIM)
        
        print("✅ Model loaded and wrapped with POS features")
...
def train_model():
    """Resume training function for Gemma with checkpoint support."""
    print("🔄 RESUMING Gemma-7B Training with POS Features")
    print("=" * 80)
    
    device = setup_gpu()
    
    # Check for existing checkpoint
    latest_checkpoint = find_latest_checkpoint(Config.MODEL_OUTPUT_DIR)
    ...
    if latest_checkpoint:
        ...
        # Only create output directory if no checkpoint exists
    ...
    model, tokenizer = load_model_from_checkpoint(
        latest_checkpoint if latest_checkpoint else Config.MODEL_NAME,
        Config.MODEL_NAME
    )
    ...
    # Save training config only if not resuming
    ...
    print(f"\n💾 Saving final model to {Config.MODEL_OUTPUT_DIR}")
    ...
    print("\n" + "="*60)
    print("🎉 GEMMA-7B WITH POS FEATURES TRAINING COMPLETED!")
    print("="*60)
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print("Usage: python train_gemma_resume.py <dataset_path> <output_dir>")
        sys.exit(1)
