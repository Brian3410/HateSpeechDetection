...
class Config:
    ...
    # Training settings - Consistent with DistilBERT
    ...
    # Data preprocessing - Same as DistilBERT
    TRAIN_SPLIT = 0.8
    VAL_SPLIT = 0.1
    TEST_SPLIT = 0.1
...
def train_model():
    """Enhanced training function for Gemma with LoRA and imbalance handling."""
    print("🚀 Starting Enhanced Gemma-7B Training for Hate Speech Detection with Imbalance Handling")
    print("=" * 80)
    ...
    with open(report_path, 'w') as f:
        f.write("Gemma-7B Hate Speech Detection - Test Set Results\n")
        f.write("=" * 60 + "\n\n")
    ...
    print("\n" + "="*60)
    print("🎉 ENHANCED GEMMA-7B TRAINING WITH IMBALANCE HANDLING COMPLETED!")
    print("="*60)
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        Config.DATASET_PATH = sys.argv[1]
        Config.MODEL_OUTPUT_DIR = "gemma_" + os.path.splitext(os.path.basename(Config.DATASET_PATH))[0] + "_model"
    else:
        print("Usage: python train_gemma.py <dataset_path> <output_dir>")
        sys.exit(1)
