...
class Config:
    ...
    MODEL_OUTPUT_DIR = "roberta_pos_only_model"
...
def train_model():
    print("🚀 Starting RoBERTa Training with POS Features Only (No SMOTE, No Class Weights)")
    print("=" * 80)
    device = setup_gpu()
    ...
    with open(report_path, 'w') as f:
        f.write("RoBERTa with POS Features Only - Hate Speech Detection Test Results\n")
        f.write("=" * 70 + "\n\n")
    ...
    print("\n" + "="*60)
    print("🎉 ROBERTA WITH POS FEATURES ONLY TRAINING COMPLETED!")
    print("="*60)
...
if __name__ == "__main__":
    # Usage: python train_roberta_pos_only.py <dataset_path> <output_dir>
    if len(sys.argv) >= 3:
        Config.DATASET_PATH = sys.argv[1]
        Config.MODEL_OUTPUT_DIR = sys.argv[2]
    elif len(sys.argv) == 2:
        Config.DATASET_PATH = sys.argv[1]
        Config.MODEL_OUTPUT_DIR = "roberta_pos_only_" + os.path.splitext(os.path.basename(Config.DATASET_PATH))[0] + "_model"
    else:
        print("Usage: python train_roberta_pos_only.py <dataset_path> <output_dir>")
        sys.exit(1)
